...
function renderIdeas(ideas) {
  ...
  return ideas.map((idea,i) => `<div class="idea-card">
    <div class="idea-lat" style="background:${BG[i%BG.length]}">
      ...
      <span class="idea-lat-num">${limpiarTexto(String(idea.numero||i+1))}</span>
    </div>
...
function generarPlantillaPDF(textoDiagnostico, plan7Dias, plan30Dias, contenidos, escenarios, mensajes) {
  ...
  const caratulaEjecucion = hayPlan ? `
    <div class="cover-interna">
      ...
      <div class="diag-title">Mapa de<br>Ejecución</div>
      <div class="description">Arquitectura de decisiones, plan de acción y ejecución comercial para los próximos 30 días.</div>
...
app.post("/*", async (req, res) => {
  ...
  try {
    ...
    const pdfBuffer = await page.pdf({
      ...
      footerTemplate: `<div style="font-size:11px;width:100%;color:#555555;padding:0 80px;display:flex;justify-content:space-between;align-items:center;font-family:'Inter',sans-serif;letter-spacing:1px;-webkit-print-color-adjust:exact;print-color-adjust:exact;"><span style="font-weight:700;color:#dc2626;letter-spacing:3px;text-transform:uppercase;">PROBLEMA CERO</span><span style="color:#888;font-weight:400;">PÁGINA <span class="pageNumber"></span></span></div>`
    });
