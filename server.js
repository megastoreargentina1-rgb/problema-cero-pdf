...
// Quita ** y * del markdown
function sinNegritas(t) {
...
function procesarMarkdownAHTML(textoCrudo) {
  ...
  // Buffers visuales
  let diasBuffer = [], semanasBuffer = [], ideasBuffer = [], ideaActual = null;
  let siEntoncesBuffer = [], mensajesBuffer = [];
  let enDias = false, enSemanas = false, enIdeas = false;
  let enSiEntonces = false, enMensajes = false;
  ...
  const prefijosIgnorar = [
    ...
    "🚀 Etapa privada","🧠 Para armar","🔎 Feedback","Del 1 al 10",
  ...
  const REGEX_TITULO = /^(?:[🧭🎯🛑🔧📅📆📌💬📊⚠️🧠⚡🔴🚀💰🔥👉⚠🔎]\s*)?(MAPA EJECUTIVO|PRIORIDAD ABSOLUTA|QUÉ DEJAR DE HACER YA|QUÉ CORREGIR PRIMERO|PLAN DE ACCIÓN|CONTENIDO QUE DEBERÍA CREAR|MENSAJES DE VENTA|MÉTRICA QUE DEBERÍA MIRAR|SI \/ ENTONCES|CIERRE ESTRATÉGICO|RESUMEN RÁPIDO|PROBLEMA PRINCIPAL|QUÉ SIGNIFICA|CAUSA REAL|ACCIÓN CONCRETA|IMPACTO|CIERRE)/i;

  function volcar() {
    if (enDias && diasBuffer.length) { html += renderDias(diasBuffer); diasBuffer=[]; enDias=false; }
    if (enSemanas && semanasBuffer.length) { html += renderSemanas(semanasBuffer); semanasBuffer=[]; enSemanas=false; }
    if (enIdeas) {
      if (ideaActual) { ideasBuffer.push(ideaActual); ideaActual=null; }
      if (ideasBuffer.length) { html += renderIdeas(ideasBuffer); ideasBuffer=[]; }
      enIdeas=false;
    }
    if (enSiEntonces && siEntoncesBuffer.length) { html += renderSiEntonces(siEntoncesBuffer); siEntoncesBuffer=[]; enSiEntonces=false; }
    if (enMensajes && mensajesBuffer.length) { html += renderMensajes(mensajesBuffer); mensajesBuffer=[]; enMensajes=false; }
  }
  ...
  lineas.forEach(linea => {
    ...
    if (limpia.startsWith('¿Tenés más TIEMPO') || limpia.startsWith('¿Este análisis') ||
        limpia.startsWith('¿Qué punto') || limpia.startsWith('Del 1 al')) { return; }

    const esTitulo = REGEX_TITULO.test(limpia);
    ...
    // CARÁTULA ANÁLISIS COMPLETO
    if (/^ANÁLISIS COMPLETO:?$/i.test(limpia)) {
      volcar();
      if (enLista) { html+='</ul>'; enLista=false; }
      if (enCajaNaranja) { html+='</div>'; enCajaNaranja=false; }
      if (enCajaCierre) { html+='</div></div>'; enCajaCierre=false; }
      html+=`<div class="cover-interna">
    ...
    if (limpia.includes("ESTE DIAGNÓSTICO ES SOLO EL PRIMER NIVEL")) {
      ...
      if (enCajaNaranja) { html+='</div>'; enCajaNaranja=false; }
      enCajaCierre=true;
      html+='<div class="seccion-nueva"><div class="contenedor-cierre"><div class="caja-premium-cierre">';
      html+='<h2 class="cierre-titulo">ESTE DIAGNÓSTICO ES SOLO EL PRIMER NIVEL</h2>';
      return;
    }
    ...
    if (limpia.includes("ESTE DIAGNÓSTICO ES SOLO EL PUNTO DE PARTIDA") ||
      ...
      if (enLista) { html+='</ul>'; enLista=false; }
      if (enCajaNaranja) { html+='</div>'; enCajaNaranja=false; }
      if (enCajaCierre) { html+='</div></div></div>'; enCajaCierre=false; }
      html+='<div class="seccion-nueva"><div class="contenedor-cierre"><div class="black-box-cta">';
      html+='<h3>TU SIGUIENTE NIVEL DE EJECUCIÓN</h3>';
      html+='<p>Detectar el bloqueo es vital, pero la transformación ocurre en la acción.</p>';
      html+='<a href="https://problemacero.com.ar" class="btn-premium">DESBLOQUEAR RUTA DE 30 DÍAS</a>';
      html+='</div></div></div>';
      ignorarResto=true; return;
    }

    // TU PRÓXIMO PASO
    if (limpia.includes("TU PRÓXIMO PASO:") || limpia.includes("**TU PRÓXIMO PASO:**")) {
      volcar();
      if (enLista) { html+='</ul>'; enLista=false; }
      html+='<div class="caja-cta-blanca"><p class="cta-titulo">TU PRÓXIMO PASO:</p>';
      enCajaNaranja=true; return;
    }

    // TÍTULOS DE SECCIÓN
    const mTit = limpia.match(/^(?:[🧭🎯🛑🔧📅📆📌💬📊⚠️🧠⚡🔴🚀💰🔥👉⚠🔎]\s*)?(MAPA EJECUTIVO|PRIORIDAD ABSOLUTA|QUÉ DEJAR DE HACER YA|QUÉ CORREGIR PRIMERO|PLAN DE ACCIÓN[^a-z]*|CONTENIDO QUE DEBERÍA CREAR|MENSAJES DE VENTA[^a-z]*|MÉTRICA QUE DEBERÍA MIRAR|SI \/ ENTONCES|CIERRE ESTRATÉGICO|RESUMEN RÁPIDO|PROBLEMA PRINCIPAL|QUÉ SIGNIFICA|CAUSA REAL|ACCIÓN CONCRETA|IMPACTO|CIERRE)$/i);
    ...
    if (mTit) {
      ...
      // Activar buffers
      enDias       = tit.includes("7 DÍAS") || (tit.startsWith("PLAN DE ACCIÓN") && !tit.includes("30"));
      enSemanas    = tit.includes("30 DÍAS") || (tit.startsWith("PLAN DE ACCIÓN") && tit.includes("30"));
      enIdeas      = tit.includes("CONTENIDO QUE DEBERÍA CREAR");
      enSiEntonces = tit.includes("SI / ENTONCES");
      enMensajes   = tit.includes("MENSAJES DE VENTA");
      ...
      html+=`<div class="seccion-nueva">
        <div class="kicker">${limpiarTexto(kicker)}</div>
        <h2 class="editorial-title">${limpiarTexto(tit)}</h2>
    ...
    if (limpia.startsWith('👉')) {
      if (enLista) { html+='</ul>'; enLista=false; }
      html+=`<p class="subtitulo-seccion">${limpiarTexto(sinNegritas(limpia.replace('👉','').trim()))}</p>`;
      return;
    }

    // ── CAPTURA DÍAS ──
    // Formato: "- **Día 1:** texto" o "- Día 1: texto"
    if (enDias) {
      const m = limpia.match(/^[-—*]\s*\*?\*?D[ií]a\s+(\d+)\*?\*?[:\s]+(.+)/i);
      if (m) { diasBuffer.push({ numero:m[1], texto:sinNegritas(m[2]) }); return; }
    }

    // ── CAPTURA SEMANAS ──
    // Formato: "- **Semana 1:** **Objetivo:** X **Acción:** Y"
    if (enSemanas) {
      const m = limpia.match(/^[-—*]\s*\*?\*?Semana\s+(\d+)\*?\*?[:\s]+(.+)/i);
      if (m) {
        const resto = sinNegritas(m[2]);
        // Extraer objetivo y acción
    ...
    // ── CAPTURA IDEAS ──
    // Formato: "- **Idea 1:**" luego "- **Gancho:** X" etc
    if (enIdeas) {
      const mNum = limpia.match(/^[-—*]\s*\*?\*?Idea\s+(\d+)\*?\*?[:\s]*$/i);
      ...
      if (mG && ideaActual) { ideaActual.gancho=sinNegritas(mG[1]); return; }
      if (mT && ideaActual) { ideaActual.tema=sinNegritas(mT[1]); return; }
      if (mO && ideaActual) { ideaActual.objetivo=sinNegritas(mO[1]); return; }
    }

    // ── CAPTURA SI/ENTONCES ──
    // Formato: "- **Si** condición, **entonces** acción"
    if (enSiEntonces) {
      const m = limpia.match(/^[-—*]\s*\*?\*?Si\*?\*?\s+(.*?),?\s+\*?\*?entonces\*?\*?\s+(.*)/i);
      if (m) { siEntoncesBuffer.push({condicion:sinNegritas(m[1]),accion:sinNegritas(m[2])}); return; }
    }
    ...
    if (enMensajes) {
      if (limpia.match(/^[-—*]\s*["""]/)) {
        const msg = limpia.replace(/^[-—*]\s*/,'').replace(/^["""]/,'').replace(/["""]$/,'').trim();
        if (msg) { mensajesBuffer.push(sinNegritas(msg)); return; }
      }
      if (limpia.startsWith('- ') || limpia.startsWith('— ')) {
        const msg = limpia.substring(2).replace(/^["""]/,'').replace(/["""]$/,'').trim();
        if (msg) { mensajesBuffer.push(sinNegritas(msg)); return; }
      }
    }
    ...
    if (limpia.startsWith('- ') || limpia.startsWith('* ') || limpia.startsWith('— ')) {
      if (!enLista) {
        html += enCajaCierre ? '<ul class="cierre-list">' : '<ul class="editorial-list">';
        enLista=true;
      }
      const itemTexto = limpiarTexto(limpia.substring(2).replace(/\*\*(.*?)\*\*/g,'<strong>$1</strong>').replace(/\*(.*?)\*/g,'$1'));
      html+=`<li class="list-item">${itemTexto}</li>`;
      return;
    } else if (enLista) { html+='</ul>'; enLista=false; }

    // PÁRRAFOS
    if (!limpia.startsWith('<')) {
      const p = limpiarTexto(limpia.replace(/\*\*(.*?)\*\*/g,'<strong>$1</strong>').replace(/\*(.*?)\*/g,'$1'));
      if (enCajaNaranja)     html+=`<p class="cta-texto">${p}</p>`;
      else if (enCajaCierre) html+=`<p class="texto-cierre">${p}</p>`;
      else                   html+=`<p class="texto-editorial">${p}</p>`;
    }
  });
  ...
  if (enCajaCierre) html+='</div></div></div>';
  return html;
}
...
function renderDias(dias) {
  ...
  dias.forEach(d=>{
    h+=`<div class="tl-item">
      ...
      <div class="tl-card"><span class="tl-texto">${limpiarTexto(d.texto)}</span></div>
    </div>`;
  });
  return h+'</div>';
}
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    .cover,.cover-interna{
      ...
      background-color:var(--negro);color:#fff;padding:60px;page-break-after:always;
    }
    ...
    .editorial-title{color:#111;font-size:36px;font-weight:800;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px;}
    ...
    .timeline-rail{position:absolute;left:28px;top:12px;bottom:12px;width:4px;background:linear-gradient(to bottom,#dc2626,#333);border-radius:2px;}
    .tl-item{position:relative;margin-bottom:14px;min-height:66px;display:flex;align-items:center;}
    .tl-nodo{position:absolute;left:-86px;width:64px;height:64px;background:var(--rojo);border-radius:50%;display:flex;flex-direction:column;align-items:center;justify-content:center;box-shadow:0 4px 16px rgba(220,38,38,.4);}
    ...
    .tl-card{background:#fafafa;border:1px solid #e8e8e8;border-left:4px solid var(--rojo);border-radius:0 6px 6px 0;padding:16px 20px;flex:1;display:flex;align-items:center;}
    ...
    .se-flecha{background:var(--rojo);width:52px;flex-shrink:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;}
